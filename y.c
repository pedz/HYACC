  ...
  if (config->marker >= 0 &&
    ...
    if (isNonTerminal(scanned_symbol) == TRUE) {
      ...
      for (r = scanned_symbol->ruleIDList; r != NULL; r = r->next) {
        ...
        // If not an existing config, add to state s.
        if (isExistingSuccessorConfig(
              s, r->ruleID, & tmp_context) == FALSE) {
          addSuccessorConfigToState(s, r->ruleID, & tmp_context);
        }
...
void combineCompatibleConfig(State * s) {
  ...
  Configuration * c;
  if (s == NULL) {
    //printf("combineCompatibleConfig: warning: s is NULL\n");
    return;
  }
  if (s->config_count <= 1) return;

  if (DEBUG_COMB_COMP_CONFIG) {
    yyprintf3("combineCompatibleCfg (state %d). before: %d, ",
           s->state_no, s->config_count);
    //writeState(s);
  }
  ...
  // shrink state configs to remove those NULL ones
  // after the last step.
  i = 0; j = 0;
  while (j < s->config_count) {
    while (s->config[j] == NULL) { j ++; }
    if (j >= s->config_count) break;
...
void getClosure(State * s) {
  ...
  xx ++;
  yy += s->config_count;
  if (zz < s->config_count) zz = s->config_count;

  if (USE_COMBINE_COMPATIBLE_CONFIG)
    combineCompatibleConfig(s);

  yyy += s->config_count;
  if (zzz < s->config_count) zzz = s->config_count;
...
void expandParsingTable() {
  ...
  //yyprintf2("expand parsing table size to %d\n", PARSING_TABLE_SIZE);
}
...
#else
  yyprintf2("%d states in total.\n", xx);
  yyprintf4("before combine: total cfg: %d, max cfg: %d, cfg/state: %.2f\n",
       yy, zz, ((double) yy) / xx); // before combineCompatibleConfig.
  yyprintf4("after combine: total cfg: %d, max cfg: %d, cfg/state: %.2f\n",
       yyy, zzz, ((double) yyy) /xx);
#endif
...
void writeStateInfo(State * s) {
  ...
  yyprintf2("state %d\n\n", s->state_no);
  //yyprintf3("  [config count : %d, core_config count : %d]\n\n",
...
void writeStateInfoFromParsingTbl() {
  ...
  for (row = 0; row < ParsingTblRows; row ++) {
    if (isReachableState(row) == TRUE) {
      ...
      //writeStateConflictList(getActualState(row));
